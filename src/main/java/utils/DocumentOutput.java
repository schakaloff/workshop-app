...
public class DocumentOutput {
    ...
    public static void printOrPdf(String title, String fxmlPath, InitFn initFn, Window owner) throws Exception {
        ...
        ButtonType btnPdf = new ButtonType("Export PDF");
        ButtonType btnCancel = ButtonType.CANCEL;

        Alert a = new Alert(Alert.AlertType.CONFIRMATION, "Choose output:", btnPrint, btnPdf, btnCancel);
        a.setTitle(title);
        a.setHeaderText(null);

        ButtonType chosen = a.showAndWait().orElse(btnCancel);
        if (chosen == btnCancel) return;
    ...
    private static void printNode(Parent node, Window owner) {
        ...
        // scale to fit page (like your existing code)
    ...
    private static void exportNodeToPdf(Parent node, Window owner, String suggestedName) throws Exception {
        ...
        fc.setInitialFileName(suggestedName.replaceAll("[^a-zA-Z0-9-_ ]", "").trim() + ".pdf");
